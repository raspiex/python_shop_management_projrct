...
class InventoryPage:
    ...
    def __init__(self, account_window):
        ...
        account_window.config(background='#ffffff')

        # ====== MENU BAR ==========
        logoIcon = Image.open('images//admin2.jpeg')
        photo = ImageTk.PhotoImage(logoIcon)
        logo = Label(account_window, image=photo, bg='#f6f6f9')
        logo.image = photo
        logo.place(x=30, y=15)
        ...
        logoIcon = Image.open('images//admin1.jpeg')
        photo = ImageTk.PhotoImage(logoIcon)
        logo = Label(account_window, image=photo, bg='#f6f6f9')
        logo.image = photo
        logo.place(x=00, y=260)
        def show_all_guest():
            conn = sqlite3.connect("A_database.db")
            cur = conn.cursor()
        ...
        # offeeImage1 = Image.open('images//coffee3.gif')
        # photo1 = ImageTk.PhotoImage(coffeeImage1)
        # coffee_image = Label( image=photo1, bg='#ffffff')
        ...
        employee_tree.heading("Month", text="Month", anchor=W)
        employee_tree.heading("Monthly_sell", text="Monthly sell", anchor=W)
        employee_tree.heading("SELL", text="Total Sell", anchor=W)

        employee_tree.column("#0", stretch=NO, minwidth=0, width=0)
        employee_tree.column("#1", stretch=NO, minwidth=0, width=40)
        employee_tree.column("#2", stretch=NO, minwidth=0, width=150)
        employee_tree.column("#3", stretch=NO, minwidth=0, width=150)
        employee_tree.column("#4", stretch=NO, minwidth=0, width=140)
        show_all_employee()
