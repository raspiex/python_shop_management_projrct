...
class AccountPage:
    ...
    def __init__(self, AccountSystem_window):
        ...
        # window Icon
        logoIcon2 = Image.open('images//coffee3.jpeg')
        icon = ImageTk.PhotoImage(logoIcon2)
        # icon = PhotoImage(file='images//icon1.jpeg')
        AccountSystem_window.iconphoto(True, icon)

        AccountSystem_window.title('THE COFFEE HOUSE')
        ...
        logo.image = photo
        logo.place(x=160, y=0)

        # ====== LOGO ==========
        coffeeImage = Image.open('images//coffee3.jpeg')
        photo = ImageTk.PhotoImage(coffeeImage)
        coffee_image = Label(landing_page, image=photo, bg='#ffffff')
        coffee_image.image = photo
        coffee_image.place(x=20, y=180)


        # Login Button
        login_button = Button(landing_page, text='Login', bg='#fd6a36', font=("", 22, "bold"), bd=0, fg='white',
                              cursor='hand2', activebackground='#fd6a36', activeforeground='white',
                              command=lambda: show_frame(sign_in))
        login_button.place(x=502, y=260)

        # Sign Up Button
        signUp_button = Button(landing_page, text='Create New Account', bg='#fd6a36', font=("", 15, "bold"), bd=0, fg='white',
                               cursor='hand2', activebackground='#fd6a36', activeforeground='white',
                               command=lambda: show_frame(sign_up))
        signUp_button.place(x=460, y=350)
        ...
        def login_all():
            ...
            if result2:
                messagebox.showinfo("Success", 'Logged in Successfully to guest,\n\nClick "OK" to continue.')
                open_guest()
            elif result1:
                messagebox.showinfo("Success", 'Logged in Successfully to admin,\n\nClick "OK" to continue.')
                open_admin()
            elif result3:
                messagebox.showinfo("Success", 'Logged in Successfully to employee,\n\nClick "OK" to continue.')
                open_employee()
            else:
